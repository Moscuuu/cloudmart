#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Create Kubernetes Secrets and update ConfigMap patches for prod environment.
...
#   - tofu/terraform state accessible (run from infra/environments/prod)
#   - Target namespace "prod" must exist
#
# Usage:
#   cd infrastructure/terraform/environments/prod
#   bash ../../../../k8s/overlays/prod/create-secrets.sh
#
...
# OAuth credentials (optional -- use empty string if not set)
...
echo ""
echo "Next steps:"
echo "  1. Apply kustomize overlay: kubectl apply -k k8s/overlays/prod/"
