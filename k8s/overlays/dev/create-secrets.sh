#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Create Kubernetes Secrets and update ConfigMap patches for dev environment.
...
#   - tofu/terraform state accessible (run from infra/environments/dev)
#   - Target namespace "dev" must exist
#
# Usage:
...
# OAuth credentials (optional for dev -- use empty string if not set)
...
echo ""
echo "Next steps:"
echo "  1. Apply kustomize overlay: kubectl apply -k k8s/overlays/dev/"
