...
> **NOTE: Bastion already has tools installed.** The Ansible playbook pre-installs kubectl, helm, and other tools on the bastion. If you are setting up a fresh bastion, see `infrastructure/ansible/playbooks/setup-bastion.yml`.
...
| 3 | Bastion | `kubectl` times out | Add `--internal-ip` to `get-credentials` |
| 4 | Platform | cert-manager CRDs not installed | Use `crds.enabled=true`, not `installCRDs` |
| 5 | Platform | Monitoring OOM on small nodes | CPU requests pre-tuned for e2-medium |
| 6 | Platform | Grafana password unknown | Default is `prom-operator` unless changed |
| 7 | Platform | Alertmanager missing slack secret | Null receiver configured as default |
| 8 | Build | Frontend env vars not available at runtime | Rebuild with `--build-arg` |
| 9 | Seeding | Network policy blocks seed pods | Add `app: product-service` label |
| 10 | Seeding | `kubectl run -i` fails via SSH tunnel | Use ConfigMap+Job pattern |
| 11 | Seeding | Spring Boot re-runs data.sql | `sql.init.mode: never` in GKE profile |
| 12 | Seeding | Alembic `%` in passwords | Fixed in code -- reads URL from env |
| 13 | OAuth | Google rejects bare IPs | Use nip.io (dashes, not dots) |
| 14 | OAuth | Missing redirect URI | Add to BOTH origins AND redirect URIs |
